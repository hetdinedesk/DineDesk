...
// Cafe-style SVG icons with stroke for the new aesthetic
const Clock = ({ size = 12, className = "" }) => (
...
const MapPin = ({ size = 12, className = "" }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={className}>
...
export const UtilityBelt = ({ isDark }) => {
  ...
  const renderItem = (key) => {
    ...
    switch (key) {
      case 'contact-info':
        return (
          <div key={key} className="flex flex-wrap items-center justify-center gap-10 font-sans text-[10px] font-medium tracking-[0.2em] uppercase">
            {/* Operating Hours */}
            {primaryLocation?.operatingHours && (
              <div className="flex items-center gap-2.5">
                <Clock size={12} />
                <span>
                  {Array.isArray(primaryLocation.operatingHours)
                    ? primaryLocation.operatingHours[0]?.open && primaryLocation.operatingHours[0]?.close
                      ? `${primaryLocation.operatingHours[0].open} — ${primaryLocation.operatingHours[0].close}`
                      : ''
                    : typeof primaryLocation.operatingHours === 'string'
                      ? primaryLocation.operatingHours
                      : ''
                  }
                </span>
            ...
            {primaryLocation?.address?.street && (
              ...
              >
                <MapPin size={12} />
                <span>
                  {[primaryLocation.address.street, primaryLocation.address.suburb].filter(Boolean).join(', ')}
                </span>
              </a>
            )}
            {/* Phone */}
      ...
      case 'reviews':
        ...
        const averageRating = googleReviewsConfig?.averageRating || reviewsConfig?.averageRating || (hasGoogleReviews ? googleReviews.reduce((acc, r) => acc + r.stars, 0) / googleReviews.length : 0);
        const totalReviews = googleReviewsConfig?.totalReviews || reviewsConfig?.totalReviews || googleReviews.length;

        if (reviewsConfig?.enableHeader !== false && hasValidPlaceId && averageRating > 0 && totalReviews > 0) {
  ...
  return (
    <div
      className="w-full py-3 px-6 z-[60]"
      style={{
    ...
    >
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex flex-wrap items-center justify-center gap-10 font-sans text-[10px] font-medium tracking-[0.2em] uppercase">
          {order.filter(k => k === 'contact-info').map(key => renderItem(key))}
        </div>

        {/* Right side: Promo message or Social/CTAs */}
        {siteConfig?.specialOffer || siteConfig?.promoText ? (
          <div className="hidden lg:block font-serif italic text-xs tracking-wider">
            {siteConfig.specialOffer || siteConfig.promoText}
          </div>
        ) : (
          <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-6">
            {order.filter(k => ['social-links', 'header-ctas', 'reviews'].includes(k)).map(key => renderItem(key))}
          </div>
        )}
      </div>
    </div>
