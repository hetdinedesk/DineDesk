        ...
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-8">
              {items.length === 0 ? (
                ...
                <div className="space-y-6">
                  {items.map(item => (
                    <div key={item._cartKey || item.id} className="bg-white border border-[var(--color-secondary)]/10 rounded-[32px] p-6 space-y-4 shadow-sm">
            ...
            {items.length > 0 && filteredSuggestions.length > 0 && (
              <div className="px-8 py-5 border-t border-[var(--color-secondary)]/10 bg-amber-50/50">
                <div className="flex items-center gap-2 mb-3">
                  <Sparkles className="w-3.5 h-3.5 text-amber-500" />
                  <span className="text-xs font-bold tracking-widest text-amber-700 uppercase">Customers also ordered</span>
                </div>
                <div className="flex flex-col gap-2">
                  {filteredSuggestions.map(s => (
                    <div key={s.id} className="flex items-center gap-3 bg-white rounded-[20px] p-2.5 border border-amber-100">
                      {s.imageUrl
                        ? <img src={s.imageUrl} alt={s.name} className="w-10 h-10 rounded-[14px] object-cover flex-shrink-0" />
                        : <div className="w-10 h-10 rounded-[14px] bg-gray-100 flex-shrink-0" />}
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-serif italic text-[var(--color-secondary)] truncate">{s.name}</div>
