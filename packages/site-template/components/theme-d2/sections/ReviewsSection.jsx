...
export const ReviewsSection = ({ title, subtitle, content = {} }) => {
  ...
  // If reviews carousel is not enabled or no reviews, don't show anything
  ...
  return (
    <section className={`py-20 ${useAlternateStyles ? 'bg-gray-900' : 'bg-gray-50'} text-[var(--color-secondary)]`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {showHeader && (
          ...
          >
            {displayTitle && (
              <h2
                className={`font-serif text-4xl md:text-5xl font-bold mb-4 ${
                  useAlternateStyles ? 'text-white' : 'text-[var(--color-secondary)]'
                }`}
            ...
            {displaySubtitle && (
              <p className={`text-xl ${
                useAlternateStyles ? 'text-gray-300' : 'text-[var(--color-secondary)]/80'
              }`}>
                {displaySubtitle}
              </p>
            )}
            ...
            {siteConfig?.reviews?.carouselContent && (
              <p className={`text-lg mt-4 ${
                useAlternateStyles ? 'text-gray-400' : 'text-[var(--color-secondary)]/60'
              }`}>
                {siteConfig.reviews.carouselContent}
              </p>
            )}
          ...
          >
            ...
            {[...activeReviews, ...activeReviews].map((review, index) => (
              <motion.div
                ...
                className={`flex-shrink-0 w-80 rounded-[2rem] p-8 shadow-md hover:shadow-xl transition-shadow duration-300 ${
                  useAlternateStyles ? 'bg-gray-800' : 'bg-white border border-gray-200 hover:border-[var(--color-primary)]/30'
                }`}
              >
                <Quote className="absolute top-8 right-10 text-[var(--color-primary)]/20 w-16 h-16" />
                ...
                <div className="flex items-center mb-6">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      ...
                      className={`${
                        i < review.rating 
                          ? (useAlternateStyles ? 'text-yellow-400 fill-current' : 'fill-[var(--color-primary)] text-[var(--color-primary)]')
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                  <div>
                    ...
                    useAlternateStyles ? 'text-white' : 'text-gray-900'
                  }`}>{review.author}</p>
                    <p className={`text-sm ${
                      useAlternateStyles ? 'text-gray-400' : 'text-[var(--color-primary)]'
